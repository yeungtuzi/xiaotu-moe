...
size_t get_amx_packed_size(enum ggml_type type, int K, int N) {
    auto get_tensor_size = [&]() -> size_t {
        size_t row_size_B{0};
         
        switch (type) {
            case GGML_TYPE_Q4_0: {
                constexpr int blck_size = QK4_0;
                row_size_B = get_row_size<block_q4_0, blck_size>(K);
                break;
            }
            case GGML_TYPE_Q4_1: {
                constexpr int blck_size = QK4_1;
                row_size_B = get_row_size<block_q4_1, blck_size>(K);
                break;
            }
            case GGML_TYPE_Q8_0: {
                constexpr int blck_size = QK8_0;
                row_size_B = get_row_size<block_q8_0, blck_size>(K);
                break;
            }
            case GGML_TYPE_Q4_K: {
                constexpr int blck_size = QK_K;
                row_size_B = get_row_size<block_q4_K, blck_size>(K);
                break;
            }
            case GGML_TYPE_Q5_K: {
                constexpr int blck_size = QK_K;
                row_size_B = get_row_size<block_q5_K, blck_size>(K);
                break;
            }
            case GGML_TYPE_Q6_K: {
                constexpr int blck_size = QK_K;
                row_size_B = get_row_size<block_q6_K, blck_size>(K);
                break;
            }
            case GGML_TYPE_IQ4_XS: {
                constexpr int blck_size = QK_K;
                row_size_B = get_row_size<block_iq4_xs, blck_size>(K);
                break;
            }
            default: 
                return 0;
...
void convert_weight_to_amx_format(
    ...
    size_t expected_size = get_amx_packed_size(type, K, N);
    
    switch (type) {
        case GGML_TYPE_Q4_0: {
            constexpr int blck_size = QK4_0;
            convert_B_packed_format<block_q4_0, blck_size>(
                packed_data, (const block_q4_0*)original_data, N, K);
            break;
        }
        case GGML_TYPE_Q4_1: {
            constexpr int blck_size = QK4_1;
            convert_B_packed_format<block_q4_1, blck_size>(
                packed_data, (const block_q4_1*)original_data, N, K);
            break;
        }
        case GGML_TYPE_Q8_0: {
            constexpr int blck_size = QK8_0;
            convert_B_packed_format<block_q8_0, blck_size>(
                packed_data, (const block_q8_0*)original_data, N, K);
            break;
        }
        case GGML_TYPE_Q4_K: {
            constexpr int blck_size = QK_K;
            convert_B_packed_format<block_q4_K, blck_size>(
                packed_data, (const block_q4_K*)original_data, N, K);
            break;
        }
        case GGML_TYPE_Q5_K: {
            constexpr int blck_size = QK_K;
            convert_B_packed_format<block_q5_K, blck_size>(
                packed_data, (const block_q5_K*)original_data, N, K);
            break;
        }
        case GGML_TYPE_Q6_K: {
            constexpr int blck_size = QK_K;
            convert_B_packed_format<block_q6_K, blck_size>(
                packed_data, (const block_q6_K*)original_data, N, K);
            break;
        }
        case GGML_TYPE_IQ4_XS: {
            constexpr int blck_size = QK_K;
            convert_B_packed_format<block_iq4_xs, blck_size>(
                packed_data, (const block_iq4_xs*)original_data, N, K);
            break;
        }
        default: 
            memcpy(packed_data, original_data, expected_size);
            break;
    }
}
 
size_t get_amx_desired_workspace_size(
    enum ggml_type type,
    int M,  
    int K,   
    int N   
...
#define LAUNCH_TINYGEMM_KERNEL_AVX_1(MB_SIZE, NB_SIZE)                                \
    tinygemm_kernel_avx<float, type, float, MB_SIZE, NB_SIZE, blck_size>::apply(    \
        K, (const float *)input_data + mb_start * K,                                \
        (const type *)weight_data + nb_start * K,                                    \
        (float *)output_data + mb_start * ldc + nb_start, ldc);

#define LAUNCH_TINYGEMM_KERNEL_VNNI_1(NB_SIZE)                                         \
    tinygemm_kernel_vnni<vec_dot_type, type, float, 1, NB_SIZE, blck_size>::apply(   \
        KB, (const char *)input_data + 0 * row_size_A,                                    \
        (const char *)weight_data + PACKED_INDEX(nb * kTilesN, 0, KB, TILE_SIZE),     \
        (float *) output_data + 0 * N + nb_start, ldc)
...
void amx_gemm_compute(
    ...
    if(is_floating_type){
        ...
        GGML_DISPATCH_FLOATING_TYPES(TYPE, [&] {
            for (int i = 0; i < MB*NB; ++i) {
                ...
                switch (mb_size << 4 | nb_size) {
                    case 0x12: LAUNCH_TINYGEMM_KERNEL_AVX_1(1, 2); break;
                    case 0x14: LAUNCH_TINYGEMM_KERNEL_AVX_1(1, 4); break;
                    case 0x16: LAUNCH_TINYGEMM_KERNEL_AVX_1(1, 6); break;
                    case 0x22: LAUNCH_TINYGEMM_KERNEL_AVX_1(2, 2); break;
                    case 0x24: LAUNCH_TINYGEMM_KERNEL_AVX_1(2, 4); break;
                    case 0x26: LAUNCH_TINYGEMM_KERNEL_AVX_1(2, 6); break;
                    case 0x32: LAUNCH_TINYGEMM_KERNEL_AVX_1(3, 2); break;
                    case 0x34: LAUNCH_TINYGEMM_KERNEL_AVX_1(3, 4); break;
                    case 0x36: LAUNCH_TINYGEMM_KERNEL_AVX_1(3, 6); break;
                    case 0x42: LAUNCH_TINYGEMM_KERNEL_AVX_1(4, 2); break;
                    case 0x44: LAUNCH_TINYGEMM_KERNEL_AVX_1(4, 4); break;
                    case 0x46: LAUNCH_TINYGEMM_KERNEL_AVX_1(4, 6); break;
                    default: fprintf(stderr, "Unexpected block size!\n");
                }
            }
        }); 
    ...
    if (M == 1) {
        ...
        GGML_DISPATCH_QTYPES(TYPE, [&] {
            ...
            for (int i = 0; i < NB; ++i) {
                ...
                switch (nb_size) {
                    //case 160: LAUNCH_TINYGEMM_KERNEL_VNNI(160); break;
                    case 128: LAUNCH_TINYGEMM_KERNEL_VNNI_1(128); break;
                    case 96: LAUNCH_TINYGEMM_KERNEL_VNNI_1(96); break;
                    case 64: LAUNCH_TINYGEMM_KERNEL_VNNI_1(64); break;
                    case 32: LAUNCH_TINYGEMM_KERNEL_VNNI_1(32); break;
                    default: fprintf(stderr, "Unexpected n block size!\n");
                }
            }
        });
    ...
    GGML_DISPATCH_QTYPES(TYPE, [&] {
        ...
        for (int i = 0; i < MB * NB; ++i) {
            ...
            tinygemm_kernel_amx<vec_dot_type, type, float, blck_size>(
                mb_size, nb_size, KB,
                (const char *)input_data + mb_start * row_size_A,
                (const char *)weight_data + PACKED_INDEX(nb * 2, 0, KB, TILE_SIZE),
                (float *) output_data + mb_start * N + nb_start, ldc);
        }
    });
