#!/usr/bin/env python
"""Engine-level A/B of xiaotu-moe MOE_MXFP4 vs REAL DeepSeek-V4-Flash layer-1
routed experts, using the fork's EXACT feeding: raw fp8_e8m0 uint8 scales
(block=32 along K) + groupK=32 + NO global scale. This mirrors what the fork's
_process_mxfp4 feeds lk_moe, and what real lk_moe.MOE_MXFP4 consumes (verified:
median ratio 1.0001 vs torch truth). Golden = dequant E2M1[nibble]*2^(byte-127).

Pure CPU (numpy only), .search-venv python. Uses prep npz (which stores s13/s2
as raw e8m0 needs regenerating), but this script re-reads safetensors itself is
done in conda; here we use the npz which has fp32 g13/g2. So we build e8m0 from
the model again via _prep. Simpler: regenerate npz to include raw uint8 e8m0.
"""
import os
import sys

import numpy as np

REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
NPZ = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "real_layer1_model.npz")
K = 6


def f32_to_bf16_bits(x):
    v = x.astype(np.float32).view(np.uint32)
    lsb = (v >> 16) & 1
    return ((v + 0x7FFF + lsb) >> 16).astype(np.uint16)


def bf16_bits_to_f32(b):
    return (b.astype(np.uint32) << 16).view(np.float32)


def main():
    d = np.load(NPZ)
    w13, w2 = d["w13"], d["w2"]
    gol13, gol2 = d["gol13"], d["gol2"]
    E, I, H = int(d["E"]), int(d["I"]), int(d["H"])

    # build raw e8m0 uint8 scales from the fp32 block scales in npz
    # (g13 = npz is fp32 2^(e-127); convert back to byte via log2+127)
    g13 = d["g13"]
    g2 = d["g2"]
    def f32_to_e8m0(s):
        lg = np.round(np.log2(np.maximum(s, 1e-30))).astype(np.int32) + 127
        return np.clip(lg, 0, 255).astype(np.uint8)
    s13 = f32_to_e8m0(g13)  # [E][2I][H/32] uint8
    s2 = f32_to_e8m0(g2)    # [E][H][I/32] uint8

    sys.path.insert(0, REPO)
    import xiaotu_moe
    m = xiaotu_moe.load()
    cfg = m.MOEConfigV2()
    cfg.num_processes = 1; cfg.process_id = 0; cfg.gpu_id = 0
    cfg.has_gate_proj = True; cfg.expert_num = E; cfg.top_k = K
    cfg.hidden_size = H; cfg.intermediate_size = I
    cfg.max_batch_size = 256; cfg.max_num_seqs = 256
    cfg.stride = 32; cfg.group_min_len = 10; cfg.group_max_len = 4096 + 128
    cfg.groupN = 1; cfg.groupK = 32; cfg.activation_type = 0
    engine = m.MOE_MXFP4(cfg, w13, w2, s13, s2, 0, 0)  # no global scale

    rng = np.random.default_rng(7)
    M = 6
    x_u16 = f32_to_bf16_bits(rng.standard_normal((M, H)).astype(np.float32))
    xf = bf16_bits_to_f32(x_u16)
    ids = rng.integers(0, E, size=(M, K)).astype(np.int32)
    wts = rng.uniform(-1, 1, size=(M, K)).astype(np.float32)
    out = np.zeros((M, H), dtype=np.float32)
    engine.cpu_prefill(M, K, ids, wts, x_u16, out)

    # golden
    golden = np.zeros((M, H), dtype=np.float32)
    for t in range(M):
        for r in range(K):
            e, w = ids[t, r], wts[t, r]
            if w == 0.0:
                continue
            g = gol13[e, 0:I] @ xf[t]
            u = gol13[e, I:2 * I] @ xf[t]
            act = u * (g / (1.0 + np.exp(-g)))
            act_bf16 = bf16_bits_to_f32(f32_to_bf16_bits(act))
            golden[t] += w * (gol2[e] @ act_bf16)

    ad = np.abs(out - golden)
    rel = ad / (np.abs(golden) + 1e-6)
    big = np.abs(golden) > 0.05
    print(f"xiaotu-moe MOE_MXFP4 (raw e8m0 uint8 scale, gk=32) vs REAL layer-1")
    print(f"  E={E} M={M} K={K}")
    print(f"  max abs err : {ad.max():.6e}")
    print(f"  mean rel err: {rel.mean():.6e}")
    print(f"  max rel (|golden|>0.05): {rel[big].max():.6e}")
    ok = rel[big].max() < 2e-3
    print(f"PASS ({'OK' if ok else 'FAIL'})  max rel(|g|>0.05) < 2e-3")
    return 0 if ok else 1


if __name__ == "__main__":
    sys.exit(main())
